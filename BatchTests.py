...
class TransactionBench:
    ...
    def __init__(self, args):
        ...
        print(f'Written serial datafile to filesystem in {time.strftime("%H:%M:%S", time.gmtime(time.time() - start))}')
        start = time.time()
        if len(all_files) > 1:
        ...
        print(f'Loaded data to database serially in {time.strftime("%H:%M:%S", time.gmtime(time.time() - start))}')
        ...
        start = time.time()
        self.load_data(all_files)
        print(f'Loaded data to database in parallel in {time.strftime("%H:%M:%S", time.gmtime(time.time() - start))}')
        self.delete_files(all_files)
        start = time.time()
        self.create_indexes()
